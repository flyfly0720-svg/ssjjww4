...
from yap_taz_model import simulate, steady_state_Y, yap_taz_odes
...
가정하기 때문입니다. 이 가정 덕분에 $h_0(t)$의 구체적 형태를 몰라도 $\beta$를 추정할 수 있습니다
...
""")
    ...
    covariate_options = {
        "HE_BMI": "체질량지수 (BMI, kg/m²)",
        "pa_aerobic": "유산소 신체활동 실천율 (0=미실천, 1=실천)",
        "HE_glu": "공복혈당 (mg/dL)",
        "age": "나이 (Cox 모형에서는 time과 별개로 공변량 포함 가능)",
        "female": "성별 (0=남성, 1=여성)",
    }
    selected = st.multiselect(
        "독립변수(공변량) 선택",
        options=list(covariate_options.keys()),
        default=["HE_BMI", "pa_aerobic", "HE_glu", "female"],
    ...
    if len(selected) == 0:
        ...
        summary["변수"] = summary.index.map(lambda x: covariate_options.get(x, x))
        summary_display = summary[["변수", "coef", "exp(coef)", "exp(coef) lower 95%",
        ...
        labels = [covariate_options.get(v, v) for v in summary.index]
...
""")
        ...
        # ---- Kaplan-Meier 곡선 (신체활동 유무별) ----
        if "pa_aerobic" in selected or "pa_aerobic" in df.columns:
            st.subheader("카플란-마이어 생존곡선 (신체활동 실천 여부별)")
            st.markdown("""
Cox 모형이 '평균적인 위험 배수'를 알려준다면, 카플란-마이어 곡선은
나이가 들수록 '암이 없을 확률(무진단 생존 확률)'이 실제로 어떻게 떨어지는지를
그대로 보여줍니다.
""")
            fig_km = go.Figure()
            for val, name, color in [(0, "신체활동 미실천", "orangered"), (1, "신체활동 실천", "royalblue")]:
                mask = df["pa_aerobic"] == val
                kmf = KaplanMeierFitter()
                kmf.fit(df.loc[mask, "time"], df.loc[mask, "event"], label=name)
                surv = kmf.survival_function_
                fig_km.add_trace(go.Scatter(
                    x=surv.index, y=surv[name],
        ...
        st.session_state["cox_summary"] = summary
...
""")
    ...
    with col_b:
        ...
        A = st.slider(
            ...
            help="개인의 비만도·공복혈당 수준을 반영하는 입력값입니다. BMI가 높거나 "
                 ...
                 "있습니다. Cox 회귀분석 탭에서 얻은 HE_BMI, HE_glu의 위험비가 클수록 "
                 "이 파라미터를 더 크게 설정하는 것이 합리적입니다."
        )
        M = st.slider(
            "M — 운동 유래 AMPK 활성 강도",
            min_value=0.0, max_value=3.0, value=0.5, step=0.05,
            help="개인의 신체활동(운동) 수준을 반영하는 입력값입니다. 유산소 운동을 "
                 ...
                 "이 값이 커진다고 볼 수 있습니다. Cox 회귀분석 탭의 pa_aerobic 변수와 "
                 "개념적으로 대응됩니다."
        )
        r = st.slider(
            "r — 암세포 기저 증식률",
    ...
    fig_ode.update_layout(
        ...
        xaxis_title="시간 t",
        yaxis=dict(title="YAP/TAZ 농도 Y(t)", titlefont=dict(color="darkorange"),
                   tickfont=dict(color="darkorange")),
        yaxis2=dict(title="암세포 수 C(t)", titlefont=dict(color="firebrick"),
    ...
    fig_mstar.update_layout(
        title=r"Y* = αA / (βM + γ)  —  운동 강도가 커질수록 YAP 정상상태가 감소",
        xaxis_title="운동 강도 M (AMPK 활성)",
        yaxis_title="YAP 정상상태 농도 Y*",
        template="plotly_white",
        height=400,
    )
    st.plotly_chart(fig_mstar, use_container_width=True)
...
""")
    ...
    if "cox_summary" not in st.session_state:
        st.warning("먼저 '1. Cox 비례위험회귀분석' 탭에서 분석을 실행해 주세요.")
    else:
        summary = st.session_state["cox_summary"]
...
그렇다면 실제 Cox 회귀 결과에서 `pa_aerobic`(신체활동 실천율)의 위험비는
이 예측과 일치할까요?
""")
        if "pa_aerobic" in summary.index:
            hr_pa = summary.loc["pa_aerobic", "exp(coef)"]
            p_pa = summary.loc["pa_aerobic", "p"]
            lower_pa = summary.loc["pa_aerobic", "exp(coef) lower 95%"]
            upper_pa = summary.loc["pa_aerobic", "exp(coef) upper 95%"]
...
관측된 HR(신체활동)이 1보다 크다는 것은, 정상상태 근사에서 다음을 의미합니다.
""")
        st.latex(r"\frac{d}{dM}\left[\delta Y^{*}(M) C\right] > 0 \quad \text{(모델의 기본 가정과 반대 부호)}")
        st.markdown(r"""
